...
public class BasicItemController {
    ...
    public String saveV4(Item item) {
        itemRepository.save(item);
        return "/basic/item";  //심각한 오류 발생 -> 중복오류
    }

    //@PostMapping("/add")
    public String saveV5(Item item) {
        itemRepository.save(item);
        return "redirect:/basic/items" + item.getId();   //PRG 패턴 (Post-Redirect-Get)
                                            //위처럼 넣는 것은 좋은 해결방법이 아님
    }
    ...
    public String saveV6(Item item, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addAttribute("status", true);    //남은건 쿼리 파라미터로 넘어감
    ...
    public void init() {
        ...
        itemRepository.save(new Item("itemB", 20000, 300));
    }
}
